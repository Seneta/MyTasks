...
- (void)setDefaultValues
{
    ...
    if (!_strokeColor) {
        _strokeColor = [UIColor colorWithRed: 21.0/255.0 green: 126.0/255.0 blue: 251.0/255.0 alpha: 0.5f];
    }
    if (!_pointFillColor) {
        _pointFillColor = [UIColor colorWithRed: 21.0/255.0 green: 126.0/255.0 blue: 251.0/255.0 alpha: 1.0f];
    }
    if (!self.graphWidth) {
        self.graphWidth = self.frame.size.width * 2;
    }
    if (!self.backgroundViewColor) {
        self.backgroundViewColor = [UIColor colorWithRed: 185.0/255.0 green: 217.0/255.0 blue: 251.0/255.0 alpha: 1.0f];
    }
    if (!self.barColor) {
    ...
    if (!self.labelFontColor) {
        self.labelFontColor = [UIColor darkGrayColor];
    }
...
- (void)plotGraphData
{
    ...
    [self setContentSize:CGSizeMake(self.graphWidth, (self.frame.size.height - 120))];
    [self addSubview:_graphView];
    
    NSInteger xCoordOffset = (self.graphWidth / [_graphData count]) / 2;
    [_graphView setFrame:CGRectMake(0 - xCoordOffset, 0, self.graphWidth, (self.frame.size.height - 120))];
    ...
    for (NSUInteger counter = 1; counter <= [_graphData count]; counter++) {
        ...
        float screenHeight = ((self.frame.size.height - 120) - (offsets)) / ((self.frame.size.height - 120) + offSetFromTop + offsetFromBottom);
        
        CGPoint point = CGPointMake(xCoord,
                                    (self.frame.size.height - 120) - (([[_graphData objectAtIndex:counter - 1] integerValue] * 
                                                               (((self.frame.size.height - 120) * screenHeight) / range)) - 
                                                              (lowest * (((self.frame.size.height - 120) * screenHeight) / range ))+
                                                              offsetFromBottom));
                          ...
                          withXAxisLabelIndex:(NSInteger)indexNumber
{
    ...
    [self setContentSize:CGSizeMake(self.graphWidth - x, (self.frame.size.height - 120))];
    
    UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(0 , 0, (self.graphWidth / [_graphData count]) - kGapBetweenBackgroundVerticalBars, (self.frame.size.height - 120) * 2)];
